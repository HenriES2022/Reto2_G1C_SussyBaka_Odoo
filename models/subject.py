...
class Subject(models.Model):
    ...
    #A method to verify that the level field don't have numbers
    @api.constrains('century')
    def _check_level_not_numbers_constrains(self):
        if not re.match("^[A-Z][a-z]{0,9}$", str(self.level)):
            raise ValidationError("Level field only can have alphabetical characters and not numbers")
